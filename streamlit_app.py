...
# ==================== PASO 2: DISEÑO ====================
    ...
    try:
        ...
        f_info = ImageFont.truetype("Canaro-Medium.ttf", 90)
        f_info_peq = ImageFont.truetype("Canaro-Medium.ttf", 70)
    except:
    ...
    w_card = 1300
    h_card = 1000
    y_card = canvas_h - h_card - 700
    
    rgb_t = tuple(int(color_tarjeta.lstrip('#')[i:i+2], 16) for i in (0, 2, 4))
    draw.rounded_rectangle([canvas_w - w_card, y_card, canvas_w + 100, y_card + h_card], radius=80, fill=rgb_t + (230,))
    
    margin_x = canvas_w - w_card + 80
    margin_y = y_card + 80
    
    draw.text((margin_x, margin_y), "📍 " + st.session_state.lugar_nombre, font=f_info, fill="white")
    draw.text((margin_x, margin_y + 120), st.session_state.lugar_dir, font=f_info_peq, fill="white")
