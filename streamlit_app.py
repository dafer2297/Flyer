...
# --- FUNCIÓN DE FONDO (CORREGIDA PARA TABLETS Y PC) ---
def set_bg_hack(main_bg):
    ...
    st.markdown(
        ...
        .stApp {{
            ...
            background-size: 100% auto; /* Ajusta al ancho para no cortar el logo */
            background-repeat: no-repeat;
            background-position: top center;
            background-attachment: scroll;
            background-color: #1E3A8A; /* Azul de respaldo para pantallas muy largas */
        }}
        /* EMPUJA EL CONTENIDO ABAJO (350px) PARA RESPETAR EL LOGO DEL FONDO */
        .block-container {{
            padding-top: 350px !important; 
        }}
        /* TEXTOS DE LA INTERFAZ EN BLANCO */
        .stMarkdown, .stText, h1, h2, h3, h4, p, label {{
            color: #FFFFFF !important;
        }}
        /* CUADROS DE TEXTO (INPUTS) MÁS LEGIBLES */
        ...
        /* BOTONES ESTILO MAGENTA */
        div.stButton > button {{
            background-color: #D81B60; /* Magenta Prefectura */
            color: white;
            border: none;
            font-weight: bold;
            border-radius: 10px;
        ...
        /* Ocultar menú de hamburguesa de Streamlit para que se vea más App */
...
try:
    ...
    st.warning("⚠️ Sube la imagen 'fondo_app.png' a GitHub (1920x1080px).")

# --- FUNCIONES GRÁFICAS (LOGICA 4K) ---
...
if st.session_state.paso == 1:
    st.info("ℹ️ Completa los campos para generar el arte oficial.")
    
    st.session_state.titulo = st.text_area("TÍTULO DEL EVENTO:", "TE INVITA")
...
# ==================== PASO 2: RESULTADO ====================
    ...
    st.success("¡Flyer Generado en 4K!")
    
    with st.expander("🎨 Ajustes de Color (Opcional)"):
    ...
    # 3. Tarjeta Info (Con ajuste automático de texto)
    w_card = 1200
    h_card = 950
    ...
    # Dibujado inteligente de textos
    ...
    # 4. Logo Visit Azuay (Abajo Izquierda)
    try:
    ...
    st.image(img_final, caption="Vista Previa (La descarga será calidad máxima)", width=400)
    
    c1, c2 = st.columns(2)
    with c1: 
        if st.button("⬅️ EDITAR DATOS"): st.session_state.paso = 1; st.rerun()
    with c2:
        buf = io.BytesIO()
        img_final.save(buf, format="PNG")
        st.download_button("📥 DESCARGAR 4K (PNG)", data=buf.getvalue(), file_name="flyer_prefectura_4k.png", mime="image/png")
